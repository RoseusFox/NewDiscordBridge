    ...
    class Bridge
    {
        ...
        public static async void SendMessage(string message)
        {
            try
            {
                ...
                if (regex.Matches(message).Count > 0 || regex.Matches(message).Count > 0)
                {
                    string newstr = regex.Replace(message, "");
            ...
            {
                TShock.Log.ConsoleError($"Error when sending message to discord: {a.Message}");
                TShock.Log.Error($"Error when sending message to discord: {a.Message}");
            }
        }
